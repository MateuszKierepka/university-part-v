...
class User{
    ...
    public function saveDB($db){
        ...
        $userName = $db->getMySqli()->real_escape_string($this->userName);
        $password = $db->getMySqli()->real_escape_string($this->password);
        $fullName = $db->getMySqli()->real_escape_string($this->fullName);
        $email = $db->getMySqli()->real_escape_string($this->email);

        $sql = "INSERT INTO users (username, passwd, fullname, email, date, status) 
                VALUES ('$userName', '$password', '$fullName', '$email', '$date', $this->status)";
